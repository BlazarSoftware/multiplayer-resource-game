...
- **Unit tests** (`test/unit/`): battle/ (calculator, status, field effects, abilities, held items, AI, calculator RNG, battle items), data/ (creature instance, creature creation, battle item def, stat tracker), world/ (season manager, shop system, social system, location system, calendar events, quest system), ui/ (compass math), crafting/ (validation), audio/ (audio manager)
...
- **AudioManager testing**: Load script onto Node, call `_create_audio_nodes()` directly (not `_ready()` — it has `_is_server()` guard). For tests needing audio nodes, use `add_child_autofree()`. Note: `_is_server()` returns true in GUT's tree (default OfflineMultiplayerPeer), so methods with server guards can't be called on tree-attached instances — test internal state directly.
...
- `scripts/autoload/` — NetworkManager, GameManager, PlayerData, SaveManager, AudioManager
...
- `test/unit/` — audio/, battle/, data/, world/, ui/, crafting/
